...
![](diagrams/positionalencodings.png)
```python
...
The mean and standard deviation are calculated over the last D dimensions of the input tensor, where D is the dimension of `normalized_shape` in our case `d_model`.
That is this will take the last 512 (if `d_model`=512) values, calculate their mean and variance, and then for each value they will subtract the the mean and divide by std deviation. 
...
- Encoder -
This will be N repetitions of encoder block,
```python
...
Same as encoder, This has some changes in its attention block.
...
- Transformer block-
This will take the input and output embeddings along with input and output positional encodings, then calculate the output from the positional embeddings and pass it to encoder and decoder. 
```python
